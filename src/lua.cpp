...
static int dochunk (lua_State *L, int status) {
  if (status == ThreadStatus::LUA_OK) status = docall(L, 0, 0);
    ...
    switch (option) {
      case 'e':  case 'l': {
        ...
        status = (option == 'e')
                 ...
                 : dolibrary(L, extra);
        if (status != ThreadStatus::LUA_OK) return 0;
        break;
      }
      case 'W':
...
static int handle_luainit (lua_State *L) {
  ...
  if (init == NULL) return ThreadStatus::LUA_OK;
  else if (init[0] == '@')
...
static void doREPL (lua_State *L) {
  ...
  while ((status = loadline(L)) != -1) {
    ...
    if (status == ThreadStatus::LUA_OK) l_print(L);
